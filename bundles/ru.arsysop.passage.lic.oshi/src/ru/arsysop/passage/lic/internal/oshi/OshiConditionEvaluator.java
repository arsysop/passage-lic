...
@Component( property={LicensingProperties.LICENSING_CONDITION_TYPE + '=' + OshiHal.LICENSING_CONDITION_TYPE_HARDWARE})
public class OshiConditionEvaluator implements ConditionEvaluator {
	...
	public Iterable<FeaturePermission> evaluate(Iterable<ConditionDescriptor> conditions) {
		...
		for (ConditionDescriptor condition : conditions) {
			...
			for (String key : keySet) {
				...
				passed = evaluateCheck(key, value);
				if (!passed) {
	...
	private boolean evaluateCheck(String key, String value) {
		switch (key) {
